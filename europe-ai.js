document.addEventListener('DOMContentLoaded',function(){
  ...
  function filtered(){
    ...
    return data.filter(function(x){
      ...
      if(q){
        var hay=[x.company,x.group,x.city,x.country,x.market,x.industry,x.wedge].join(' ').toLowerCase();
  ...
  [search,country,market,employees,revenue,tier].forEach(function(el){el.addEventListener(el===search?'input':'change',render)});
  reset.addEventListener('click',function(){search.value='';country.value='';market.value='';employees.value='';revenue.value='';tier.value='';sortKey='rank';sortDir=1;render()});
  ...
  exportBtn.addEventListener('click',function(){
    var rows=filtered();
    var cols=['rank','company','group','country','city','market','industry','employees','employeeRange','revenue','revenueBand','wedge','tier'];
